...
function sign(player) { return jwt.sign({ id: player._id.toString(), role: 'player', name: player.name, email: player.email }, process.env.JWT_SECRET, { expiresIn: '12h' }); }
...
router.post('/register', async (req, res) => {
  try {
    ...
    const player = await Player.create({ name, email, passwordHash });
